...
class FaceExpressionNet(nn.Module):
    ...
    def forward(self, x):
        x = self.conv1(x)
        x = self.conv2(x)
        x = self.conv3(x) # 輸出尺寸 BATCH x 256 x 8 x 8

        # ⭐ 1. 計算 GAP 和 GMP
        x_avg = self.avg_pool(x) # BATCH x 256 x 1 x 1
        x_max = self.max_pool(x) # BATCH x 256 x 1 x 1
        
        # ⭐ 2. 沿通道維度 (dim=1) 串聯
        x_combined = torch.cat([x_avg, x_max], dim=1) # BATCH x 512 x 1 x 1
        
        # 3. 展平
        x = x_combined.view(x_combined.size(0), -1) # BATCH x 512
